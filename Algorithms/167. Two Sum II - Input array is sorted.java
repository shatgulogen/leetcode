class Solution {
    public int[] twoSum(int[] numbers, int target) {
        int lo = 0, hi = numbers.length -1;// 2 pointers one at the beginning index, the other point to the last index number
        ...
        while(lo < hi) {// the lo pointer should not pass the hi pointer
            ...
            if (currentSum == target) {
                return new int[] { lo+1, hi+1}; // the index in the output starts with 1 instead of 0, so we need to add 1 to the pointers in the new reserved array
            } else if (currentSum > target) {
                hi--; // if the current sum is greater than the target, we move the hi pointer to the left because the array is sorted with right side //numbers always greater than the left side numbers, and so the number which the hi pointer is pointing at will decrease and so the target will decrease as well. We name the pointer lo and hi instead of left and right because it also shows the value
            } else {
